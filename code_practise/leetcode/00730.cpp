...
public:
    /*
    Consider palindrom a_a in substring S[i,j], where S[i] = S[j] is a.
    a_a could be using S[i]_S[j], or could be any a in substring _.
    Since we only consider unique palindroms, we should only count the one
    using S[i]_S[j].
